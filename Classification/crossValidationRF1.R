...
show('File name is : crossValidationRF1.R - Choose this File from Stored Folder Path')
filename = "crossValidationRF1.R"
filepath=1
...
rawData=read.csv("trainData1.csv",header=FALSE)
rawDataLabel=read.csv("trainLabel1.csv",header=FALSE)[,1]
...
#sdVar=lapply(rawData, sd)
#if(length(sdvar)>0){
#lowSdIdx = which(unlist(sdVar)==0)
#rawData = rawData[,-lowSdIdx]
#}
#Generate random indices to shuffle the samples
...
gridDF = expand.grid(.mtry=c(15),ntree=c(1000,100))
...
best_model = randomForest(x=rawData ,y=as.factor(rawDataLabel), mtry=best_params$.mtry,ntree=best_params$ntree,importance=TRUE)
 ...
 testDataRaw = read.csv("testData1.csv",header = FALSE)
...
write.table(testPreds,file="PunyamurthulaClassificationCV1.txt",row.names = FALSE,col.names = FALSE)
show("Predictions file created. Please check file PunyamurthulaClassificationCV1.txt in the following Path")

show(dir)

}else{
  show('Please choose the File mentioned : crossValidationRF1.R')
}
