...
show('File name is : RUN_Missing_Value_Estimation_1.R - Choose this File from Stored Folder Path')
filename = "RUN_Missing_Value_Estimation_1.R"
filepath=1
...
rawData = read.csv("mve1.csv",header=FALSE)
...
gridDF = expand.grid(lam=c(0),rank=c(2))

errorList = c()
for(i in 1:nrow(gridDF)){
  
  softImputedValues = softImpute(rawDataM,lambda=gridDF$lam[i])
...
softImputedValues = softImpute(actualDataMadeMissing,lambda=0,rank.max = 2)
umat=((softImputedValues["u"][[1]]))
dmat =((softImputedValues["d"][[1]]))
vmat = ((softImputedValues["v"][[1]]))
...
write.table(missingImputedFinal, file="PunyamurthulaMissingResult1.txt",row.names = FALSE,col.names = FALSE,sep="\t")

show("Predictions file created. Please check file PunyamurthulaMissingResult1.txt in the following Path")

show(dir)

}else{
  show('Please choose the File mentioned : RUN_Missing_Value_Estimation_1.R')
}
